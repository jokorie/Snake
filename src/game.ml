...
(* Exercise 02b:
   ...
   Note the signature of this function: {[ val set_direction : Snake.t ->
   Direction.t -> unit ]}
...
let handle_key t key =
  let snake_dir = Direction.of_key key in
    match snake_dir with
    | None -> ()
    | Some snake_dir -> Snake.set_direction t.snake snake_dir
;;
...
(* Exercise 06b:
   ...
   called. It should check if the snake head is at the current position of
   the apple stored in the game.

   Hint: We've given you some functions to help with this. Take a look at
   snake.mli and apple.mli to find functions to get the positions you need to
   consider

   If it is, we should call the [grow_over_next_steps] function in snake.ml
   that we just implemented to update the snake so that it can grow over the
   next few time steps. The amount it should grow is based on the value of
...
let maybe_consume_apple t =
  let snake_pos = Snake.head (snake t) in
  let apple_pos = Apple.position t.apple in
  if Position.equal snake_pos apple_pos
  then (
    Snake.grow_over_next_steps (snake t) (Apple.amount_to_grow t.apple);
    let new_apple = Apple.create ~board:t.board ~snake:t.snake in
    match new_apple with
    | Some pos -> t.apple <- pos
    | None -> t.game_state <- Win)
  else ()
;;

let wipe t = 
  let snake = Snake.create ~length:3 in
  let apple = Apple.create ~board:(board t) ~snake:snake in
  t.snake <- snake;
